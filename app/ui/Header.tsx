...
export function Header() {
  ...
  // Track the rendered header height so the mobile menu drop-down aligns even
  // when the utility bar wraps at narrow widths.
  useEffect(() => {
  ...
  return (
    <header ref={headerRef} className="sticky top-0 z-20 border-b border-slate-200 bg-white/90 backdrop-blur">
      <div className="bg-brand-green text-white">
        <div className="main-container flex min-h-9 items-center justify-between py-1.5 text-xs font-semibold">
      ...
      <div className="main-container flex items-center justify-between gap-4 py-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="hidden items-center gap-2 sm:flex">
            <Link href="/service-request" className="hidden lg:inline-flex rounded-md border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-700 transition-colors hover:border-brand-green hover:text-brand-green">Service Request</Link>
            <Link href="/quote" className="inline-flex rounded-md bg-brand-green px-3 py-2 text-sm font-semibold text-white transition-colors hover:bg-brand-greenDark">Request a Quote</Link>
          </div>
          <button type="button" className="inline-flex md:hidden items-center justify-center rounded-md border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700" aria-label={open ? "Close menu" : "Open menu"} aria-expanded={open} onClick={() => setOpen((v) => !v)}>{open ? "Close" : "Menu"}</button>
        </div>
      </div>
      ...
      {open ? (
        <div className="md:hidden">
          ...
          <div
            className="fixed left-0 right-0 z-30 border-b border-slate-200 bg-white shadow-lg"
            style={{ top: headerHeight }}
          >
            <nav className="main-container py-4">
              <ul className="grid grid-cols-1 gap-2">
                {utilityLinks.map((link) => (
                  <li key={link.label}><Link href={link.href} target={link.external ? "_blank" : undefined} rel={link.external ? "noreferrer" : undefined} onClick={close} className="block rounded-md px-3 py-2 text-base font-medium text-slate-800 hover:bg-brand-mist">{link.label}</Link></li>
                ))}
                {primaryNav.map((link) => (<li key={link.href}><Link href={link.href} onClick={close} className="block rounded-md px-3 py-2 text-base font-medium text-slate-800 hover:bg-brand-mist">{link.label}</Link></li>))}
                {secondaryNav.map((link) => (<li key={link.href}><Link href={link.href} onClick={close} className="block rounded-md px-3 py-2 text-base font-medium text-slate-700 hover:bg-brand-mist">{link.label}</Link></li>))}
              </ul>
            </nav>
          </div>
        </div>
